...
# Locate all inputs within Final_Runs_July2026. The exclusion and retallying
# logic below is retained from the original no-Kaplanis-overlap analysis.
...
derived_input_dir <- file.path(
  final_runs_dir, "outputs", "derived", "no_overlap", "inputs"
)
dir.create(derived_input_dir, recursive = TRUE, showWarnings = FALSE)
...
gnomad_information_v2 <- data.frame(fread(
  file.path(constraint_dir, "gnomad.v2.1.1.lof_metrics.by_gene.txt")
))

#Filter the genedx data for the kaplanis overlap
GeneDx_ped <- read.table(file.path(
  pedigree_dir,
  "GeneDx_ASC_de_novos_GRCh38_v3.4_ped_HPO_terms_removed_2025-07-05_showing_Kaplanis_overlap.txt"
), header = TRUE)
GeneDx_count <- read.table(file.path(
  count_dir,
  "GeneDx_ASC_de_novo_counts_by_gene_GRCh38_new_mis_cats_2025-07-05.txt"
), header = TRUE)
GeneDx_variant <- read.table(file.path(
  variant_dir,
  "GeneDx_ASC_de_novos_GRCh38_v3.4_calls_2025-07-05.txt"
), header = TRUE)


Overlap_Samples = GeneDx_ped$Sample[GeneDx_ped$In_Kaplanis ==1]
GeneDx_Ped_filter = GeneDx_ped[!(GeneDx_ped$Sample %in% Overlap_Samples),]
GeneDx_variant_filter = GeneDx_variant[!(GeneDx_variant$Sample %in% Overlap_Samples),]

write.table(GeneDx_Ped_filter,
            file = file.path(derived_input_dir, "GeneDx_ASC_de_novos_GRCh38_v3.4_ped_HPO_terms_removed_2025-07-05_showing_Kaplanis_overlap_OverlapRemoved.txt"),quote = FALSE, sep = "\t",col.names = TRUE, row.names = FALSE)
write.table(GeneDx_variant_filter,
            file = file.path(derived_input_dir, "GeneDx_ASC_de_novos_GRCh38_v3.4_calls_2025-07-05_OverlapRemoved.txt"),quote = FALSE, sep = "\t",col.names = TRUE, row.names = FALSE)
...
ped_files[3] <- file.path(derived_input_dir, "GeneDx_ASC_de_novos_GRCh38_v3.4_ped_HPO_terms_removed_2025-07-05_showing_Kaplanis_overlap_OverlapRemoved.txt")



count_files = file.path(count_dir, c("SPARK_iWES_v2_de_novo_counts_by_gene_new_samples_only_new_mis_cats_2025-03-29.txt",
...
variant_files[3] <- file.path(derived_input_dir, "GeneDx_ASC_de_novos_GRCh38_v3.4_calls_2025-07-05_OverlapRemoved.txt")
...
sparkiWES2_counts = read.table(file.path(count_dir, "SPARK_iWES_v2_de_novo_counts_by_gene_new_samples_only_new_mis_cats_2025-03-29.txt"),
                               header = TRUE,
                               sep = "\t")
...
autism_loop_vars$names = c("Combined Probands PTV",
...
autism_loop_vars$datasets = c(rep(list((autism_counts$phase %in% c("New","Fu2022")),
...
autism_loop_vars$N_subset = c(rep(c(sum(autism_counts$N_Proband),
...
autism_loop_vars$prevalences = c(rep(baseprev, 10))


# Weight prevalence using only the cohorts actually present in this sensitivity
# dataset. The DDD ASD cohort is deliberately absent, and GeneDx reflects the
# post-overlap-removal sample size recorded in colData.
...
N_total_no_overlap = N_SPARK + N_ASC + N_GeneDX

weight_SPARK = N_SPARK/N_total_no_overlap
weight_ASC = N_ASC/N_total_no_overlap
weight_GeneDX = N_GeneDX/N_total_no_overlap

prev_weighted = (weight_SPARK * 0.0276) +
  (weight_ASC * 0.0276) +
  (weight_GeneDX * 0.01)

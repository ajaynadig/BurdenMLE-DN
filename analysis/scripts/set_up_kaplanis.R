...
resource_dir <- file.path(
  Sys.getenv(
    "BURDENMLEDN_ANALYSIS_ROOT",
    unset = file.path(getwd(), "analysis")
  ),
  "inputs", "developmental_disorder"
)
if (!dir.exists(resource_dir)) {
  stop(
    "Developmental-disorder inputs are unavailable. Place the authorized ",
    "count_data_update_April2025 directory under: ", resource_dir
  )
}
setwd(resource_dir)
...
gnomad_information <- read.table(
  file.path(final_runs_dir, "inputs", "constraint",
...
#Define looping variables for main BurdenMLE_DN loop
...
# `prev_weighted` is defined from the current autism pedigrees by set_up_asc.R,
# which is sourced immediately before this DDD setup in the main workflow.
prev_factors = c(1,prev_weighted/0.01,0.0276/0.01)
